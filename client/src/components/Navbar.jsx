...
const Navbar = () => {
    ...
    const { unreadMessages, fetchUnreadCounts, unreadNotifications, fetchNotifications, soundEnabled, setSoundEnabled } = useSocket();
    
    // İlk takımın ID'sini bul (varsa)
    const firstTeamId = [...teams, ...ledTeams][0]?._id;
    
    // Sayfa yüklendiğinde okunmamış mesaj sayısını ve bildirimleri getir
    useEffect(() => {
        if (user && user.token) {
            console.log('Navbar: Okunmamış mesaj sayısı ve bildirimler getiriliyor...');
            fetchUnreadCounts();
            fetchNotifications();
        }
    }, [user, fetchUnreadCounts, fetchNotifications]);
